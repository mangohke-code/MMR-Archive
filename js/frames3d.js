...
window.loadFramesModel3D = function loadFramesModel3D(container, modelUrl, options = {}) {
  ...
  loader.load(modelUrl, (gltf) => {
    ...
    // FBX 원본이 항상 정면 기준 오른쪽으로 90~120도가량 돌아간 상태로 나온다 —
    // FBX2glTF 변환 시 좌표축 관례(Maya 등)와 우리가 카메라를 세팅하는 기준(+Z 방향)이
    // 어긋나는 것으로 보인다. 모든 보스에 공통이라 고정 보정값을 적용한다.
    gltf.scene.rotation.y = THREE.MathUtils.degToRad(105);
    ...
    // 이 있다 — 페이즈가 섞여서 다 같이 표시되면 모델이 겹쳐 보이므로, 페이즈 태그가 있는
    // 메시는 파츠 토글과 별도로 "현재 선택된 페이즈"에 속한 것만 보이게 관리한다.
    ...
    function applyVisibility() {
      meshes.forEach(m => {
        const p = meshPhase(m.name);
        const phaseOk = !p || p === currentPhase;
        m.visible = enabledMeshes.has(m.name) && phaseOk;
      });
    }
    applyVisibility();
