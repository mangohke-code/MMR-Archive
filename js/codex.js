  ...
  // 파일 이름, 요소 id·클래스, Supabase 표 이름(캐릭터_도감), 아래 저장 키까지 전부
  // 따라갔다. 저장 키를 갈면 저장해 둔 스토리 진행도가 없는 셈이 되어 설문 화면부터
  // 다시 봐야 하는데, 고르는 데 십 초면 되는 것이라 이름을 맞추는 쪽을 골랐다.
  const SURVEY_STORAGE_KEY = 'nikke_codex_survey';
  ...
  const SEASON_ORDER_HINT = ['일반', '메이드', '바니걸', '여름', '할로윈', '크리스마스', '신년', '콜라보'];
