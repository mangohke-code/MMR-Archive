...
JOBS = [
    ...
    ('추출프로그램 업데이트 이후/bbg008_검은 뱀 H.S.T.A. · summon.glb', 'bbg008'),
    ('S26 프로비던스/xbg002_프로비던스 Z.E.U.S.glb', 'xbg002'),
    # 거대 질량체 두 마리는 여태 이 목록에 없었다(다른 경로로 만들어 올린 듯하다).
    # 다시 만들 때 헤매지 않게 여기에 적어 둔다.
    ...
    # 출력 이름에 변종을 적는다.
    ('S39 아일랜드 이터/ebg001_island.glb', 'ebg001_island'),
    ...
    # 추출 프로그램이 갱신되면서 카메라 노드가 연출별로 갈라졌다. 예전 파일은 카메라
    # 하나를 모든 연출이 나눠 썼고, 1페이즈 파일에 2페이즈 카메라가(그 반대도) 섞여
    # 있었다. 새 추출본 폴더를 쓴다.
